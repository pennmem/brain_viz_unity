...
{
    /// <summary>
    /// Represents a <see cref="AssetLoader"/> asset loaded event handler.
    /// </summary>
    ...
    /// <summary>
    /// Represents a <see cref="AssetLoader"/> metadata processed event handler.
    /// </summary>
    ...
    public class AssetLoader : IDisposable
    {
        ...
        /// <summary>
        /// Use this field to assign the event that will occurs when each file metadata be processed.
        /// </summary>
        ...
        /// <summary>
        /// Loads the <see cref="UnityEngine.Texture"/> resource to show in case of unknown textures, if needed.
        ...
        {
            if (_normalBaseTexture == null)
            {
                _normalBaseTexture = Resources.Load("NormalBase") as Texture2D;
            }
            return _normalBaseTexture != null;
        }
        ...
        /// <param name="onAssetLoaded">Use this field to set the event that triggers when the asset is loaded.</param>
        /// <param name="error">Outputs last error message.</param>
        /// <param name="onTexturePreLoad">Use this field to set the event that triggers before a texture is loaded.</param>
        /// <param name="options"><see cref="AssetLoaderOptions"/> used to load the object.</param>
        /// <param name="wrapperGameObject">Use this field to load the new <see cref="UnityEngine.GameObject"/> into referenced <see cref="UnityEngine.GameObject"/>.</param>
        /// <returns></returns>
        public bool LoadFromMemoryWithTextures(byte[] data, string assetExtension, ObjectLoadedHandle onAssetLoaded, out string error, TexturePreLoadHandle onTexturePreLoad = null, AssetLoaderOptions options = null, GameObject wrapperGameObject = null)
        {
            ...
            {
                OnObjectLoaded += onAssetLoaded;
            }
            if (onTexturePreLoad != null)
            {
                OnTexturePreLoad += onTexturePreLoad;
            }
            else
            {
                OnTexturePreLoad += delegate (IntPtr scene, string path, string name, Material material, string propertyName, ref bool checkAlphaChannel, TextureWrapMode textureWrapMode, string basePath, TextureLoadHandle onTextureLoaded, TextureCompression textureCompression, bool isNormalMap)
                {
                    ...
                    int width;
                    int height;
                    if (Texture2DUtils.LoadEmbeddedTextureData(scene, path, out finalPath, out textureData, out isRawData, out width, out height))
                    {
                        Texture2DUtils.LoadTextureFromMemory(textureData, path, material, propertyName, ref checkAlphaChannel, textureWrapMode, onTextureLoaded, textureCompression, isNormalMap, isRawData);
                        return;
                    }
#if TRILIB_USE_ZIP
                    if (zipFile != null)
                    {
                        ...
                        foreach (ZipEntry zipEntry in zipFile)
                        {
                            ...
                            if (zipEntry.Name.ToLowerInvariant() == fileShortName)
                            {
                                ...
                                Texture2DUtils.LoadTextureFromMemory(uncompressedData, path, material, propertyName, ref checkAlphaChannel, textureWrapMode, onTextureLoaded, textureCompression, isNormalMap);
                                return;
                            }
                        }
                    }
                ...
                try
                {
                    LoadFromMemory(data, assetExtension, options, wrapperGameObject);
                }
                catch (Exception e)
                {
                    error = e.ToString();
        ...
        /// <summary>
        /// Reads the scene metadata info and calls OnMetadataProcessed, if present.
        /// </summary>
        /// <param name="scene">Previously loaded scene pointer.</param>
        private void LoadMetadata(IntPtr scene)
        {
            ...
            for (uint i = 0; i < metadataCount; i++)
            {
                ...
                OnMetadataProcessed(metadataType, i, metadataKey, metadataValue);
            }
        }
        ...
        private static void BuildDefaultMaterials()
        {
            ...
            _standardBaseMaterial.SetTexture("_BumpMap", _normalBaseTexture);
            ...
            _standardSpecularMaterial.SetTexture("_BumpMap", _normalBaseTexture);
            ...
            _standardBaseAlphaMaterial.SetTexture("_BumpMap", _normalBaseTexture);
            ...
            _standardSpecularAlphaMaterial.SetTexture("_BumpMap", _normalBaseTexture);
            ...
            _standardBaseCutoutMaterial.SetTexture("_BumpMap", _normalBaseTexture);
            ...
            _standardSpecularCutoutMaterial.SetTexture("_BumpMap", _normalBaseTexture);
        ...
        private void BuildMaterials(string filename, IntPtr scene, AssetLoaderOptions options)
        {
            ...
            for (uint m = 0; m < AssimpInterop.aiScene_GetNumMaterials(scene); m++)
            {
                ...
                else
                {
                    ...
                    var textureDiffusePath = string.Empty;
                    var textureDiffuseName = string.Empty;
                    var textureDiffuseWrapMode = TextureWrapMode.Clamp;
                    ...
                    if (numDiffuse > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            if (OnTexturePreLoad == null)
                            {
                                diffuseTexture = Texture2DUtils.LoadTextureFromFile(scene, path, textureName, null, null, ref checkAlphaChannel, wrapMode, texturesPath, null, options.TextureCompression, filenameWithoutExtension);
                                textureDiffuseLoaded = diffuseTexture != null;
                            }
                            else
                            {
                            ...
                            textureDiffusePath = path;
                            textureDiffuseWrapMode = wrapMode;
                            textureDiffuseName = textureName;
                        }
#if TRILIB_OUTPUT_MESSAGES || ASSIMP_OUTPUT_MESSAGES
						else
                        {
                    ...
                    if (!options.DisableAlphaMaterials && (hasDiffuseAlphaTexture || alpha < 1f))
                    {
                    ...
                    else
                    {
                        if (OnTexturePreLoad != null)
                        {
                            ...
                            OnTexturePreLoad(scene, textureDiffusePath, textureDiffuseName, unityMaterial, "_MainTex", ref checkAlphaChannel, textureDiffuseWrapMode, texturesPath, OnTextureLoaded, options.TextureCompression);
                        }
                        else
                        {
                            ...
                            {
                                OnTextureLoaded(textureDiffusePath, unityMaterial, "_MainTex", diffuseTexture);
                            }
                        }
                    }
                    ...
                    if (numEmissive > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            if (OnTexturePreLoad != null)
                            {
                                OnTexturePreLoad(scene, path, textureName, unityMaterial, "_EmissionMap", ref checkAlphaChannel, wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression);
                            }
                            else
                            {
                    ...
                    if (numSpecular > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            if (OnTexturePreLoad != null)
                            {
                                OnTexturePreLoad(scene, path, textureName, unityMaterial, "_SpecGlossMap", ref checkAlphaChannel, wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression);
                            }
                            else
                            {
                    ...
                    if (numNormals > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            if (OnTexturePreLoad != null)
                            {
                                OnTexturePreLoad(scene, path, textureName, unityMaterial, "_BumpMap", ref checkAlphaChannel, wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression, true);
                            }
                            else
                            {
                    ...
                    if (numHeight > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            if (OnTexturePreLoad != null)
                            {
                                OnTexturePreLoad(scene, path, textureName, unityMaterial, "_BumpMap", ref checkAlphaChannel, wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression);
                            }
                            else
                            {
